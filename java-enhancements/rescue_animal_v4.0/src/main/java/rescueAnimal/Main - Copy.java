...
public class Main {
        ...
        while (true) {
            try {
                ...
                if (age.isEmpty()) {
                    ...
                    if (!age.matches("\\d+") || Integer.parseInt(age) > 50) {
                        System.out.print(RED + "Please enter a valid age: " + RESET);
    ...
    public static String getWeight(Scanner scanner) {
        ...
        while (true) {
            try {
                ...
                } else if (!weight.matches("\\d+(\\.\\d+)?") || Float.parseFloat(weight) > 500) {
        ...
        while (true) {
            try {
                ...
                if (acquisitionDate.isEmpty()) {
                    ...
                    if (!acquisitionDate.matches("\\d{2}-\\d{2}-\\d{4}")) {  // Invalid date
    ...
    public static void intakeNewMonkey(Scanner scanner) {
        ...
        try {
            ...
            for (var entry : monkeyMap.entrySet()) {
                if (entry.getValue().getName() != null && entry.getValue().getName().equalsIgnoreCase(name)) {
    ...
    public static void reserveAnimal(Scanner scanner) {
        ...
        if (userChoice.equalsIgnoreCase("n")) return;
            ...
            if (animalType.equalsIgnoreCase("dog")) {
                try {
                    for (var entry : dogMap.entrySet()) {  // 'entry' is each key-value pair in the Map
                        // Get complete 'dog' value of the current entry and call the Dog's getInServiceCountry method
                        // Then compare that value with the desired inServiceCountry
                        if (entry.getValue().getInServiceCountry().equalsIgnoreCase(inServiceCountry)) {
            ...
            // Reserve in service monkey if available
                ...
                try {
                    for (var entry : monkeyMap.entrySet()) {  // 'entry' is each key-value pair in the Map
                        // Get complete 'monkey' value of the current entry and call the Dog's getInServiceCountry method
                        // Then compare that value with the desired inServiceCountry
                        if (entry.getValue().getInServiceCountry().equalsIgnoreCase(inServiceCountry)) {
                            entry.getValue().setReserved(true);
                            System.out.println(CYAN + "\nReserved successfully!" + RESET);
    ...
    public static void main(String[] args) {
        ...
        scanner.close();
        if (dogDB != null && !dogDB.isClosed()) { dogDB.close(); }
        if (monkeyDB != null && !monkeyDB.isClosed()) { monkeyDB.close(); }
    }
}
